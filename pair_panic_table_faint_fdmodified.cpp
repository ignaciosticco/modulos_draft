...
#include <stdlib.h>
#include <string.h>
#include "pair_panic_table_faint_fdmodified.h"
#include "atom.h"
#include "force.h"
#include "comm.h"
...
PairPanicTableFaintFdmodified::PairPanicTableFaintFdmodified(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
}
...
PairPanicTableFaintFdmodified::~PairPanicTableFaintFdmodified()
{
  if (copymode) return;
...
void PairPanicTableFaintFdmodified::compute(int eflag, int vflag)
{
...
void PairPanicTableFaintFdmodified::allocate()
{
  allocated = 1;
...
void PairPanicTableFaintFdmodified::settings(int narg, char **arg)
{
...
void PairPanicTableFaintFdmodified::coeff(int narg, char **arg)
{
...
double PairPanicTableFaintFdmodified::init_one(int i, int j)
{
...
void PairPanicTableFaintFdmodified::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairPanicTableFaintFdmodified::bcast_table(Table *tb)
{
...
void PairPanicTableFaintFdmodified::spline_table(Table *tb)
{
...
void PairPanicTableFaintFdmodified::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairPanicTableFaintFdmodified::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairPanicTableFaintFdmodified::null_table(Table *tb)
{
...
void PairPanicTableFaintFdmodified::free_table(Table *tb)
{
...
void PairPanicTableFaintFdmodified::spline(double *x, double *y, int n,
...
double PairPanicTableFaintFdmodified::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairPanicTableFaintFdmodified::write_restart(FILE *fp)
{
...
void PairPanicTableFaintFdmodified::read_restart(FILE *fp)
{
...
void PairPanicTableFaintFdmodified::write_restart_settings(FILE *fp)
{
...
void PairPanicTableFaintFdmodified::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairPanicTableFaintFdmodified::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairPanicTableFaintFdmodified::extract(const char *str, int &dim)
{
...
void PairPanicTableFaintFdmodified::initialize_acum_comp_force(int acum_comp_force[]){
...
void PairPanicTableFaintFdmodified::wall_extract(Table *tb, char *line,FILE *fp)
{
  int  i,n;
  char word[MAXLINE];
...
void PairPanicTableFaintFdmodified::compute_wallforce(double x, double y, double vx, double vy, double force_wall[], Table *tb){
...
void PairPanicTableFaintFdmodified::compute_desiredforce(double x, double y, double vx, double vy, double* force_desired, double vd, Table *tb){
...
void PairPanicTableFaintFdmodified::compute_compression_force(int i, Table *tb, double vector_vd_x[], double vector_vd_y[], double vector_compforce[], int acum_comp_force[]){


    int j,jj,jnum,inum,itype,jtype,itable;
    double xtmp,ytmp,vxtmp,vytmp,delx,dely,delz,delvx,delvy,evdwl,fpair,gpair; // Ignacio Sticco - Guillermo Frank 17 april 2018
    ...
    for (jj = 0; jj < jnum; jj++) {
        ...
        if (rsq < cutsq[itype][jtype] && rsq < DIAM2) {
            tb = &tables[tabindex[itype][jtype]];
            if (rsq < tb->innersq) {
                sprintf(estr,"Pair distance < table inner cutoff: " 
                "ijtype %d %d dist %g",itype,jtype,sqrt(rsq));
            ...
            if (tabstyle == LINEAR) {
                ...
                fraction = (rsq - tb->rsq[itable]) * tb->invdelta;
                value = tb->f[itable] + fraction*tb->df[itable];
                fpair = CEXP1* factor_lj * value;                                        // Sticco 11 May
...
void PairPanicTableFaintFdmodified::search_near_opening(int i, double vector_vd_x[], double vector_vd_y[], Table *tb){
