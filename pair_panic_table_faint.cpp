...
#include <stdlib.h>
#include <string.h>
#include "pair_panic_table_faint.h"
#include "atom.h"
#include "force.h"
#include "comm.h"
...
PairPanicTableFaint::PairPanicTableFaint(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
}
...
PairPanicTableFaint::~PairPanicTableFaint()
{
  if (copymode) return;
...
void PairPanicTableFaint::compute(int eflag, int vflag)
{
...
void PairPanicTableFaint::allocate()
{
  allocated = 1;
...
void PairPanicTableFaint::settings(int narg, char **arg)
{
...
void PairPanicTableFaint::coeff(int narg, char **arg)
{
...
double PairPanicTableFaint::init_one(int i, int j)
{
...
void PairPanicTableFaint::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairPanicTableFaint::bcast_table(Table *tb)
{
...
void PairPanicTableFaint::spline_table(Table *tb)
{
...
void PairPanicTableFaint::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairPanicTableFaint::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairPanicTableFaint::null_table(Table *tb)
{
...
void PairPanicTableFaint::free_table(Table *tb)
{
...
void PairPanicTableFaint::spline(double *x, double *y, int n,
...
double PairPanicTableFaint::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairPanicTableFaint::write_restart(FILE *fp)
{
...
void PairPanicTableFaint::read_restart(FILE *fp)
{
...
void PairPanicTableFaint::write_restart_settings(FILE *fp)
{
...
void PairPanicTableFaint::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairPanicTableFaint::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairPanicTableFaint::extract(const char *str, int &dim)
{
...
void PairPanicTableFaint::initialize_acum_comp_force(int acum_comp_force[]){
...
void PairPanicTableFaint::wall_extract(Table *tb, char *line,FILE *fp)
{
  int  i,n;
  char word[MAXLINE];
...
void PairPanicTableFaint::compute_wallforce(double x, double y, double vx, double vy, double force_wall[], Table *tb){
...
void PairPanicTableFaint::compute_desiredforce(double x, double y, double vx, double vy, double* force_desired, double vd, Table *tb){
...
void PairPanicTableFaint::compute_compression_force(int i, Table *tb, double vector_vd_x[], double vector_vd_y[], double vector_compforce[], int acum_comp_force[]){
...
void PairPanicTableFaint::search_near_opening(int i, double vector_vd_x[], double vector_vd_y[], Table *tb){
