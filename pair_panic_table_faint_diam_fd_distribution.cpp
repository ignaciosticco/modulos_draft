...
#include <stdlib.h>
#include <string.h>
#include "pair_panic_table_faint_diam_fd_distribution.h"
#include "atom.h"
#include "force.h"
#include "comm.h"
...
PairPanicTableFaintDiamFdDistribution::PairPanicTableFaintDiamFdDistribution(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
}
...
PairPanicTableFaintDiamFdDistribution::~PairPanicTableFaintDiamFdDistribution()
{
  if (copymode) return;
...
void PairPanicTableFaintDiamFdDistribution::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    vector_compforce[i] = 0.0;      // Cornes 14 Jun
  }
  ...
  if (num_pasos==500){
      num_pasos = 0;
        ...
        for (ii = 0; ii < inum; ii++) {
            i = ilist[ii];
            compute_compression_force(i, tb, vector_vd_x, vector_vd_y,vector_compforce,acum_comp_force);    //Cornes 13 Jun

            if (acum_comp_force[i] > nrun_faint) {
                type[i] = 2;
            }
        }
    }
...
void PairPanicTableFaintDiamFdDistribution::allocate()
{
  allocated = 1;
...
void PairPanicTableFaintDiamFdDistribution::settings(int narg, char **arg)
{
  ...
  initialize_acum_comp_force(acum_comp_force);              // Sticco Cornes 22 Jun
...
void PairPanicTableFaintDiamFdDistribution::coeff(int narg, char **arg)
{
...
double PairPanicTableFaintDiamFdDistribution::init_one(int i, int j)
{
...
void PairPanicTableFaintDiamFdDistribution::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairPanicTableFaintDiamFdDistribution::bcast_table(Table *tb)
{
...
void PairPanicTableFaintDiamFdDistribution::spline_table(Table *tb)
{
...
void PairPanicTableFaintDiamFdDistribution::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairPanicTableFaintDiamFdDistribution::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairPanicTableFaintDiamFdDistribution::null_table(Table *tb)
{
...
void PairPanicTableFaintDiamFdDistribution::free_table(Table *tb)
{
...
void PairPanicTableFaintDiamFdDistribution::spline(double *x, double *y, int n,
...
double PairPanicTableFaintDiamFdDistribution::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairPanicTableFaintDiamFdDistribution::write_restart(FILE *fp)
{
...
void PairPanicTableFaintDiamFdDistribution::read_restart(FILE *fp)
{
...
void PairPanicTableFaintDiamFdDistribution::write_restart_settings(FILE *fp)
{
...
void PairPanicTableFaintDiamFdDistribution::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairPanicTableFaintDiamFdDistribution::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairPanicTableFaintDiamFdDistribution::extract(const char *str, int &dim)
{
...
void PairPanicTableFaintDiamFdDistribution::initialize_acum_comp_force(int acum_comp_force[]){
    
    int *ilist;
    int vector_length = 100000;

    int inum = list->inum;                      // Cornes 14 Jun
    ilist = list->ilist;                        // Cornes 14 Jun
    int ii,i;

    for (ii = 0; ii < vector_length; ii++) {             // Cornes 14 Jun
        acum_comp_force[ii] = 0;                 // Cornes 14 Jun
    }
}


void PairPanicTableFaintDiamFdDistribution::wall_extract(Table *tb, char *line,FILE *fp)
{
  int  i,n;
  char word[MAXLINE];
...
void PairPanicTableFaintDiamFdDistribution::compute_wallforce(double x, double y, double vx, double vy, double force_wall[], Table *tb, double rad){

  double rsq,numerator,denominator,xp,yp,c1,c2,c3,c4,c5,c6,c7,c8,c9,r,cexp2,cociente;
  ...
  int *type = atom->type;
  delz=0.0;
  double rad2;      // Sticco Cornes 29-Jun 2018
...
void PairPanicTableFaintDiamFdDistribution::compute_desiredforce(double x, double y, double vx, double vy, double* force_desired, double vd, Table *tb){
  ...
  double dist2,rand1,y_rand,dist2_threshold;         // Sticco 30 Jul
  force_desired[0] = 0.0;
  force_desired[1] = 0.0;
  dist2_threshold = 3.0;                            // Sticco 30 Jul
  // random for desired velocity //         // Sticco 24 Ago
  double sigma_vd = 0.5*vd;             
  srand(time(NULL));
  vd = vd + ((double) rand() / (RAND_MAX)) - sigma_vd;
  printf("vd = %lg \n",vd );
  int j=0;
    while(j< tb->opening_ninput){
      ...
      if(orientation == 0){                    // Vertical orientation
          ...
          dx = xi - x;
          dy = (yf+yi)/2.0 - y;           // Sticco 30 Jul
          dist2 = dx*dx+dy*dy;            // Sticco 30 Jul
          if (dist2<dist2_threshold){     // Sticco 30 Jul
            if (dx*pointing > 0){       //significa que no paso por el opening
              ...
              rand1 = (rand() % 11);             // Sticco 30 Jul
              y_rand = yi + rand1/10.0 * (yf-yi);  // Sticco 30 Jul
              dy = y_rand-y;                    // Sticco 30 Jul
              ubication = 4;                    // Sticco 30 Jul
              rsq = dx*dx + dy*dy;               // Sticco 30 Jul
...
void PairPanicTableFaintDiamFdDistribution::compute_compression_force(int i, Table *tb, double vector_vd_x[], double vector_vd_y[], double vector_compforce[], int acum_comp_force[]){


    int j,jj,jnum,inum,itype,jtype,itable;
    double xtmp,ytmp,vxtmp,vytmp,delx,dely,delz,delvx,delvy,evdwl,fpair,gpair,cexp1,producto; // Ignacio Sticco - Guillermo Frank 17 april 2018
    double rsq,factor_lj,fraction,value,a,b,raiz,inv_raiz,fuerza_compresion,fuerza_compresion_x,fuerza_compresion_y;
    char estr[128];
    int *ilist,*jlist,*numneigh,**firstneigh;
    ...
    double **x = atom->x;
    double **f = atom->f;
    double **v = atom->v;                                              // Ignacio Sticco - Guillermo Frank 17 april 2018
    double *radius = atom->radius;          // Sticco Cornes 29-Jun 2018
    int *type = atom->type;
    int nlocal = atom->nlocal;
    double *special_lj = force->special_lj;
    int newton_pair = force->newton_pair;
    double diam2;                          // Sticco Cornes 29-Jun 2018     
    inum = list->inum;
    ilist = list->ilist;
    numneigh = list->numneigh;
    firstneigh = list->firstneigh;  
    ...
    search_near_opening(i,vector_vd_x, vector_vd_y, tb);  //Cornes 13 Jun
    ...
    for (jj = 0; jj < jnum; jj++) {
        ...
        producto = rsq;
        jtype = type[j];
        diam2 = (radius[i] + radius[j])*(radius[i] + radius[j]);     // Sticco Cornes 29-Jun 2018
        if (rsq < cutsq[itype][jtype] && rsq < diam2) {              // Sticco Cornes 29-Jun 2018
            tb = &tables[tabindex[itype][jtype]];
            if (rsq < tb->innersq) {
                sprintf(estr,"Pair distance < table inner cutoff: " 
                "ijtype %d %d dist %g",itype,jtype,sqrt(rsq));
            ...
            if (tabstyle == LINEAR) {
                ...
                fraction = (rsq - tb->rsq[itable]) * tb->invdelta;
                value = tb->f[itable] + fraction*tb->df[itable];
                fpair = factor_lj * value;           // Cornes 30-Jun 2018

                /////////////////////////////////////////////////////////////////
                rsq = diam2;                                                       // Cornes 30 Jun 2018
                itable = static_cast<int> ((rsq - tb->innersq) * tb->invdelta);   // Cornes 30 Jun 2018
                fraction = (rsq - tb->rsq[itable]) * tb->invdelta;                // Cornes 30 Jun 2018
                value = tb->f[itable] + fraction*tb->df[itable];                  // Cornes 30 Jun 2018
                cexp1 = 1.0/(value * sqrt(rsq));                                  // Cornes 30 Jun 2018
                ...
                value = tb->rsqt[itable] + fraction*tb->drsqt[itable];                 // Ignacio Sticco - Guillermo Frank 17 april 2018
            } 

            raiz = sqrt(producto);
            inv_raiz = 1.0 / raiz;

            search_near_opening(j,vector_vd_x, vector_vd_y, tb);  //Cornes 13 Jun
            
            //Comentario Cornes
            //fpair es la fuerza social dividida la distancia (=raiz). Por eso se proyecta solamente multiplicando por dx y dy.

            fuerza_compresion = fpair-A*inv_raiz;                                         // Cornes 14 Jun (fpair ya viene dividido por raiz)

            fuerza_compresion_x = delx*fuerza_compresion;                               // Cornes 14 Jun
            fuerza_compresion_y = dely*fuerza_compresion;                               // Cornes 14 Jun

            vector_compforce[i] += fabs(fuerza_compresion_x*vector_vd_x[i]+fuerza_compresion_y*vector_vd_y[i]);                  // Cornes 14 Jun

            if (newton_pair || j < nlocal) {      
                vector_compforce[j] += fabs(fuerza_compresion_x*vector_vd_x[j]+fuerza_compresion_y*vector_vd_y[j]);                  // Cornes 14 Jun
            }
        }
    } 
    
    if(vector_compforce[i] > umbral_faint) {
        acum_comp_force[i] += 1;
    }
    else if(vector_compforce[i] == 0.0) {
        acum_comp_force[i] = 0;
    }
...
void PairPanicTableFaintDiamFdDistribution::search_near_opening(int i, double vector_vd_x[], double vector_vd_y[], Table *tb){
